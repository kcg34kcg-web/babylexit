...
// --- 1. İÇERİK VERİ TABANI (TAMAMI KORUNDU) ---
...
// --- 2. OYUN BİLEŞENİ: JUSTICE RUNNER (TAMAMI KORUNDU) ---
...
// --- 3. ANA BİLEŞEN: LOADING OVERLAY (DÜZELTİLDİ) ---
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl relative overflow-hidden flex flex-col min-h-[500px]">
        ...
        {/* --- ORTA ALAN: İÇERİK (HER ZAMAN GÖRÜNÜR - İŞLEM BİTSE BİLE) --- */}
        ...
        {/* --- ALT KISIM (DEĞİŞEN BÖLÜM) --- */}
        
        {/* A. EĞER BİTTİYSE: SONUÇ BUTONU GÖSTER (İçerik yukarıda kalmaya devam ediyor) */}
        {isFinished ? (
            <div className="bg-emerald-950/90 p-5 border-t border-emerald-500/30 backdrop-blur-md animate-in slide-in-from-bottom-2 duration-500 absolute bottom-0 left-0 right-0 z-20">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className="bg-emerald-500/20 p-2 rounded-full"><CheckCircle className="text-emerald-400 w-6 h-6" /></div>
                        <div>
                            <h3 className="text-emerald-100 font-bold text-sm">Analiz Tamamlandı</h3>
                            <p className="text-emerald-400/70 text-xs">Yapay zeka raporunuzu hazırladı.</p>
                        </div>
                    </div>
                    
                    {onClose ? (
                        <button onClick={onClose} className="w-full md:w-auto group bg-white text-emerald-900 font-bold px-6 py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-emerald-50 transition-all shadow-lg">
                            Sonuçları Gör <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                        </button>
                    ) : (
                        <Link href={redirectUrl} className="w-full md:w-auto group bg-white text-emerald-900 font-bold px-6 py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-emerald-50 transition-all shadow-lg">
                            Sonuçları Gör <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                        </Link>
                    )}
                </div>
            ...
            /* B. EĞER DEVAM EDİYORSA: MENÜ VE YÜKLEME ÇUBUĞU GÖSTER */
            <>
                <div className="px-6 pb-2">
                    <div className="flex justify-between items-center bg-slate-950/60 rounded-xl p-1.5 border border-slate-800 backdrop-blur-sm gap-1">
                        <button onClick={() => setMode('game')} className={cn("flex-1 py-2.5 rounded-lg transition-all flex justify-center hover:bg-slate-800", mode === 'game' ? "bg-amber-600/20 text-amber-500" : "text-slate-500")}><Gamepad2 size={22} /></button>
                        <button onClick={() => setMode('article')} className={cn("flex-1 py-2.5 rounded-lg transition-all flex justify-center hover:bg-slate-800", mode === 'article' ? "bg-purple-600/20 text-purple-500" : "text-slate-500")}><BookOpen size={22} /></button>
                        <button onClick={() => setMode('fact')} className={cn("flex-1 py-2.5 rounded-lg transition-all flex justify-center hover:bg-slate-800", mode === 'fact' ? "bg-blue-600/20 text-blue-500" : "text-slate-500")}><Brain size={22} /></button>
                        <button onClick={() => setMode('quote')} className={cn("flex-1 py-2.5 rounded-lg transition-all flex justify-center hover:bg-slate-800", mode === 'quote' ? "bg-amber-600/20 text-amber-500" : "text-slate-500")}><Quote size={22} /></button>
                        <button onClick={() => setMode('breath')} className={cn("flex-1 py-2.5 rounded-lg transition-all flex justify-center hover:bg-slate-800", mode === 'breath' ? "bg-green-600/20 text-green-500" : "text-slate-500")}><Wind size={22} /></button>
                    </div>
                    <div className="text-[10px] text-center text-slate-600 mt-2 mb-1">Beklerken bir mod seçebilirsiniz</div>
                </div>
                <div className="bg-slate-950 p-4 border-t border-slate-800">
                    <div className="flex justify-between text-xs text-slate-400 font-medium mb-2">
                        <span className="flex items-center gap-2"><Loader2 className="w-3 h-3 animate-spin text-blue-500" />{STEPS[stepIndex]}</span>
                        <span className="text-blue-500 font-bold">%{Math.min((stepIndex + 1) * 20, 99)}</span>
                    </div>
                    <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-blue-600 via-purple-600 to-amber-600 transition-all duration-500 ease-out shadow-[0_0_10px_rgba(59,130,246,0.5)]" style={{ width: `${((stepIndex + 1) / STEPS.length) * 100}%` }} />
                    </div>
                </div>
            </>
        )}
      ...
      <style jsx global>{`
        @keyframes slide-infinite { 0% { right: -60px; } 100% { right: 120%; } }
        .animate-slide-infinite { animation: slide-infinite 1.8s linear infinite; }
